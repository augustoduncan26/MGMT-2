<?php
	...
	*/
	...
	// Buscar id de PRE-HOSPITALARIA
	//===============================
	$P_idDepto		=	$objMante->BuscarLoQueSea('*' ,PREFIX.'mant_departamentos', 'id_cia="'.$id_cia.'"','extract');
	...
	*/
	...
	//*****************************************************
	$P_TotUser	=	$objMante->PromUsers(PREFIX.'mant_areas','users','id_depts = "'.$PIDDEPTO.'" '); 	
	$IDSUP			=	FALSE;
	$MANTIDSUP		=	FALSE;
	$PASSASUP		=	FALSE;
	$FILA_N			=	0;
	...
	$Wer_userSuperv	=	'id_depto = "'.$PIDDEPTO.'" AND activo = "1" ORDER BY id_area,nombre ASC';
	$P_UserDptoSup	=	$objMante->Listar(PREFIX.'users', $Wer_userSuperv,false,false,false,false,'array');
	
	//var_dump($P_UserDptoSup);
	...
	{
		/*
			1. Buscar la formula de acuerdo al rea seleccionada
...
// ================================================
			...
			$SQL__			=	mysqli_query($link,'SELECT * FROM 911_rolturn_desp_tmp WHERE dpto = "'.$PIDDEPTO.'" AND activo = 1 AND area like "%'.$P_Data.'%" AND meses="'.$DIGITOS[$rrh].'" ORDER BY id '.$MIRAND[$NUMR].' LIMIT '.$POST[2]) or die('Error 504 inesperado en mysql: '.mysql_error());
...
// por id, Ejem. fila 1 ( dia 1), fila 1 ( dia 2), fila 1 ( dia 3), etc...
					...
					// ===============================
							 if($w==0):
							 		//var_dump($NCampo);
									$P_Conten   .=  '<select id="user-'.$P_Exito2['id'].'" name="user-'.$P_Exito2['id'].'" style="width:130px" onchange="javascript: OtroGuardadoRapido(\'user-'.$P_Exito2['id'].'\' ,\'DESP\', \''.$P_Exito2['id'].'\', \'id\'); NoDuplicar(\''.$POST_users.'\',\'user-'.$P_Exito2['id'].'\')" title="user-'.$P_Exito2['id'].'">';
