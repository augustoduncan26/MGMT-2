...
{
    private $exito;
    private $tableStudentAssignment;
    private $idCia;
    private $idUser;

    public function __construct()
    {
        $this->tableStudentAssignment = PREFIX.'assoc_student_assignment';
    ...
    public function save ($POST) {
		...
		$sql 		=	$ObjMante->BuscarLoQueSea('*',$this->tableStudentAssignment,'id_cia="'.$this->idCia.'" and teacher_id = "'.$POST['r1'].'"','array');
    ...
    public function delete ( $id ) {
        $ObjEjec    =   new ejecutorSQL();
        $r = $ObjEjec->ejecutarSQL("Delete from ".$this->tableStudentAssignment." Where id = '".$id."' and id_cia='".$this->idCia."'");
        if ($r == 1) {
            $this->exito = 'ok';
        } else {
            $this->exito = 'error';
