...
if ($id_user == 1 || $id_user == 2) {
  $sel        = mysql_query("Select * From ".$caja_prefix."proveedores");
} else {
  $sel        = mysql_query("Select * From ".$caja_prefix."proveedores Where id_empresa = '".$id_empresa."'");
}
    ...
    <table id="list-table-room" class="table table-striped table-bordered table-hover table-full-width">
      ...
      ?>
        <tr>
         ...
         <td width="150px" <?php if($datos->activo==0){?> class="row-yellow-transp" <?php } ?>><?=$datos->codigo?></td>
          <td width="150px" <?php if($datos->activo==0){?> class="row-yellow-transp" <?php } ?>><?=$datos->nombre?></td>
          <td width="150px" <?php if($datos->activo==0){?> class="row-yellow-transp" <?php } ?>><?=$datos->direccion?></td>
          <td width="150px" <?php if($datos->activo==0){?> class="row-yellow-transp" <?php } ?>><?=$datos->telefono?></td>
          ...
          <td width="100px" class="" style="width:15% !important;">
            ...
            <a class="btn btn-xs btn-teal tooltips" data-original-title="Consultar cuenta del proveedor" data-toggle="modal" role="button" href="#edit_room" onclick="editCategory('<?php echo $datos->id; ?>');"><i class="fa fa-dollar"></i></a>
