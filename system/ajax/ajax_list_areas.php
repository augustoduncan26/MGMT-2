<?php

$id_user    = $_GET['id_user'];
$id_empresa = $_GET['id_empresa'];

include_once ('../framework.php');
$ObjMante   = new Mantenimientos();
$sel1       = $ObjMante->BuscarLoQueSea('*',PREFIX.'mant_areas','id_cia = '.$id_empresa,'array');

?>
<div class="table-responsive">
    <table id="list-table-direcciones" class="table table-striped table-bordered table-hover">
      <thead>
        <tr class=""><!-- header-list-table -->
          <th>Nombre</th>
          <th>Departamento</th>
          <th>Tot. Usuarios</th>
          <th>Turno A</th>
          <th>Turno B</th>
          <th>Turno C</th>
          <th>Turno D</th>
          <th>Turno E</th>
          <th>Estado</th>
          <th>Fecha creación</th>
          <th></th>
        </tr>
      </thead>
      <tbody>
      <?php
        if ($sel1['resultado']){
          foreach ($sel1['resultado'] as $datos) {
            $departamento  		= $ObjMante->BuscarLoQueSea('*',PREFIX.'mant_departamentos','id_cia = '.$_SESSION['id_cia'].' and id='.$datos['id_depto'].' and active=1','extract');

      ?>
        <tr>
          <td <?php if($datos['active']==0) { echo 'class="row-yellow-transp"'; } ?>><?=$datos['name']?></td>
          <td <?php if($datos['active']==0) { echo 'class="row-yellow-transp"'; } ?>><?=$departamento['name'];?></td>
          <td <?php if($datos['active']==0) { echo 'class="row-yellow-transp"'; } ?>><?=$datos['users']?></td>
          <td <?php if($datos['active']==0) { echo 'class="row-yellow-transp"'; } ?>><?=$datos['turn_a']?></td>
          <td <?php if($datos['active']==0) { echo 'class="row-yellow-transp"'; } ?>><?=$datos['turn_b']?></td>
          <td <?php if($datos['active']==0) { echo 'class="row-yellow-transp"'; } ?>><?=$datos['turn_c']?></td>
          <td <?php if($datos['active']==0) { echo 'class="row-yellow-transp"'; } ?>><?=$datos['turn_d']?></td>
          <td <?php if($datos['active']==0) { echo 'class="row-yellow-transp"'; } ?>><?=$datos['turn_e']?></td>
          <td <?php if($datos['active']==0) { echo 'class="row-yellow-transp"'; } ?>><?php if($datos['active'] ==1) { echo 'Activo'; } else { echo '<label style="color:red">Inactivo</label>';} ?></td>
          <td <?php if($datos['active']==0) { echo 'class="row-yellow-transp"'; } ?>><?=$datos['created_at']?></td>
          <td class="text-center" style="width:10% !important;">
            <a class="btn btn-xs btn-teal tooltips" data-original-title="Ver Detalle" data-toggle="modal" role="button" href="#edit_event" onclick="editRow('<?php echo $datos['id']; ?>');"><i class="fa fa-edit"></i></a>
            <a class="btn btn-xs btn-bricky tooltips" data-original-title="Eliminar" href="Javascript:void(0);" onclick="if (confirm('Está seguro que desea eliminar este registro?')) { deleteRow('<?php echo $datos['id']; ?>'); } else { return false; }"><i class="fa fa-times fa fa-white"></i></a>
            </td>
        </tr>
     <?php
        }
      }
     ?>
       <tfoot>
         <tr></tr>
       </tfoot>
      </tbody>
    </table>
</div>